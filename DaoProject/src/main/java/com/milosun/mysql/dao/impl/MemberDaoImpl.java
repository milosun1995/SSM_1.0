...
public class MemberDaoImpl implements IMemberDao {
	...
	public boolean doCreate(Member vo) throws Exception {
		String sql="INSERT INTIO memer(mid,name,age,phone,birthday,note) VALUES (?,?,?,?,?,?)";
	...
	public boolean doUpdate(Member vo) throws Exception {
		String sql="UPDATE memer SET name=?,age=?,phone=?,birthday=?,note=? WHERE mid=?";
	...
	public boolean doRemoveBatch(Set<String> ids) throws Exception {
		...
		buf.append("DELETE FROM memer WHERE mid IN(");
	...
	public List<Member> findAllSplit(Integer currentPage, Integer lineSize) throws Exception {
		List<Member>  all =new ArrayList<Member>();
		String sql ="SELECT mid,name,age,phone,birthday,note FROM member LIMIT ?,?";
		this.pstmt=this.conn.prepareStatement(sql);
	...
	public List<Member> findAllSplit(String column, String keyWord, Integer currentPage, Integer lineSize)
			throws Exception {
		List<Member>  all =new ArrayList<Member>();
		String sql ="SELECT mid,name,age,phone,birthday,note FROM member WHERE "+column+" like ? LIMIT ?,? AND";
		this.pstmt=this.conn.prepareStatement(sql);
	...
	public Long getAllCount() throws Exception {
		String sql="SELECT COUNT(*) FROM memer";
		this.pstmt=this.conn.prepareStatement(sql);
		ResultSet rs=this.pstmt.executeQuery();
		if(rs.next()) {
			return rs.getLong(1);
	...
	public Long getAllCount(String column, String keyWord) throws Exception {
		String sql="SELECT COUNT(*) FROM memer WHERE "+column+" like ?";
		this.pstmt=this.conn.prepareStatement(sql);
		this.pstmt.setString(1, "%"+keyWord+"%");
		ResultSet rs=this.pstmt.executeQuery();
		if(rs.next()) {
